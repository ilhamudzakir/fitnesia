$(document).ready(function(){
    $('#px-site_content-news-form').validate({
        rules: {                                            
            title: {
                required: true
            },
        ...
        submitHandler: function(form) {
            ...
            $('#px-site_content-news-form .alert-warning').removeClass('hidden');
            $('#px-site_content-news-form .alert-success').addClass('hidden');
            $('#px-site_content-news-form .alert-danger').addClass('hidden');
            ...
            $.ajax({
                ...
                success : function(response){
                    $('#px-site_content-news-form .alert-warning').addClass('hidden');
                    if(response.status == 'ok'){
                        $('#px-site_content-news-form .alert-success').removeClass('hidden').children('span').text(response.msg);
                        window.location.href = response.redirect;
                    }
                    else
                        $('#px-site_content-news-form .alert-danger').removeClass('hidden').children('span').text(response.msg);   
                },
    ...
    $('#px-site_content-news-form-content').summernote({
        toolbar: [
        ['style', ['style']],
    ...
    function sendFile(file, editor, welEditable) {
        ...
        $.ajax({
            ...
            success: function(url) {
                ...
                $('#px-site_content-news-form .panel-body').after('<input type="hidden" name="images[]" value="'+url+'">');
            }
        });
    }
    $("#file-upload").fileupload({
        ...
        if (data.files.error) {
            ...
            $('#px-site_content-news-fileupload-'+target+'-progress').text(progress+'%');
            $('#px-site_content-news-fileupload-'+target+'-upload-button').attr('disabled',true);
        }).on('fileuploaddone', function (e, data) {
            var target = $('#target-file').val();
            $('#px-site_content-news-fileupload-'+target+'-upload-button').removeAttr('disabled');
                        a = 1.6;
                        w = 160;
                        b = 1 ;
                        h = 100;
            ...
            $('#px-site_content-news-fileupload-upload-button').removeAttr('disabled');
