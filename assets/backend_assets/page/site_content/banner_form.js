$(document).ready(function(){
	$('#px-site-content-banner-form').validate({
		ignore: [],
		rules: {                                            
		...
		submitHandler: function(form) {
			...
			$('#px-site-content-banner-form .alert-warning').removeClass('hidden');
			$('#px-site-content-banner-form .alert-success').addClass('hidden');
			$('#px-site-content-banner-form .alert-danger').addClass('hidden');
			$.ajax({
				url : target,
				type : 'POST',
				dataType : 'json',
				...
				success : function(response){
					$('#px-site-content-banner-form .alert-warning').addClass('hidden');
					if(response.status == 'ok'){
						$('#px-site-content-banner-form .alert-success').removeClass('hidden').children('span').text(response.msg);
						window.location.href = response.redirect;
					}
					else
						$('#px-site-content-banner-form .alert-danger').removeClass('hidden').children('span').text(response.msg);	
				},
	...
	$("#file-upload").fileupload({
		...
		if (data.files.error) {
			...
			$('#px-site-content-banner-fileupload-'+target+'-progress').text(progress+'%');
			$('#px-site-content-banner-fileupload-'+target+'-upload-button').attr('disabled',true);
		}).on('fileuploaddone', function (e, data) {
			var target = $('#target-file').val();
			$('#px-site-content-banner-fileupload-'+target+'-upload-button').removeAttr('disabled');
			...
			$('#px-site-content-banner-fileupload-upload-button').removeAttr('disabled');
